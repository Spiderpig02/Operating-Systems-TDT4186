...
int fork_together(int forkNum)
{
    ...
    if (forkNum)
    {
        // if (uvmsharedcopy(p->pagetable, np->pagetable, p->sz) < 0)
        // {
        //     freeproc(np);
        //     np->vforked = 1;
        //     release(&np->lock);
...
int fork(void)
{
    return fork_together(0);
}

// VFORK implementation
int vfork(void)
{
    return fork_together(1);
}
    ...
    for (;;)
    {
        ...
        for (pp = proc; pp < &proc[NPROC]; pp++)
        {
            if (pp->parent == p)
            {
                ...
                {
                    acquire(&pp->lock);
                    while (pp->state != ZOMBIE && pp->state != UNUSED)
                    {
                        release(&pp->lock);
                        sleep(pp, &p->lock);
                        acquire(&pp->lock);
